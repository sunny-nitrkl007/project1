...
if [ -f ${LAST_DOWNLOAD_TIME_FILE_PATH} ]
then
  LAST_DOWNLOAD_TIME=$(date --utc -r "${LAST_DOWNLOAD_TIME_FILE_PATH}" +%s)
  if [ $? -ne 0 ]
  then
    LAST_DOWNLOAD_TIME=0
  fi
...
do
  THIS_FILE_TIME=$(date --utc -r "${VP3_FILE}" +%s)
  if [ $? -ne 0 ] || [ $THIS_FILE_TIME -gt $LAST_DOWNLOAD_TIME ]
  then
